module mosart_budget_type
   ...
   use mosart_vars, only: re, spval, barrier_timers, iulog, &
   ...
   subroutine set_budget(this, begr, endr, ntracers, dt)
      ...
      do nr = begr, endr
         do nt = 1, ntracers
            ...
            this%in_grc(nr, nt) = (ctl%qsur(nr, nt) + ctl%qsub(nr, nt) + ctl%qgwl(nr, nt)) * dt
      ...
      do nr = begr, endr
         do nt = 1, ntracers
            ...
            if (ctl%mask(nr) >= 2) then
               ...
               this%lag_grc(nr, nt) = this%lag_grc(nr, nt) - ctl%erout_prev(nr, nt) &
                                      - ctl%flow(nr, nt)
            end if
            ...
            if (abs(this%net_glob(nt) + this%lag_glob(nt)) > 1e-6) then
               if (abs(this%net_glob(nt) - this%lag_glob(nt)) &
                  ...
                  relerr = abs(this%net_glob(nt) - this%lag_glob(nt)) &
                           /abs(this%net_glob(nt) + this%lag_glob(nt))
               end if
            end if
            if (mainproc) then
              write (iulog, '(a)')         '-----------------------------------'
