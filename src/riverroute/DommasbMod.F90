MODULE DommasbMod
  ...
  !----------------------------------------------------------------------
    subroutine hillslopeRoutingDOM(iunit,nt,ntdom,theDeltaT)
      ...
      Tdom%domHout(iunit,ntdom) = min((Tdom%domH(iunit,ntdom)+Tdom%domsur(iunit,ntdom)*theDeltaT)/theDeltaT,max(0._r8,min(-TRunoff%ehout(iunit,nt) * (Tdom%domH(iunit,ntdom) + Tdom%domsur(iunit,ntdom) * theDeltaT)/(max(0._r8,TRunoff%wh(iunit,nt)-TRunoff%dwh(iunit,nt)*theDeltaT)+TRunoff%qsur(iunit,nt)*theDeltaT),-TRunoff%ehout(iunit,nt) * 0.3_r8)))
    ...
    subroutine subnetworkRoutingDOM(iunit,nt,ntdom,theDeltaT)
      ...
      integer, intent(in) :: iunit, nt, ntdom
      real(r8), intent(in) :: theDeltaT
      Tdom%domTout(iunit,ntdom) = min((Tdom%domT(iunit,ntdom)+(Tdom%domsub(iunit,ntdom)+Tdom%domHout(iunit,ntdom))* theDeltaT)/theDeltaT,max(0._r8,min(-TRunoff%etout(iunit,nt) * (Tdom%domT(iunit,ntdom) + (Tdom%domsub(iunit,ntdom)+Tdom%domHout(iunit,ntdom)) * theDeltaT)/(max(0._r8,TRunoff%wt(iunit,nt)-TRunoff%dwt(iunit,nt)*theDeltaT)+TRunoff%etin(iunit,nt)*theDeltaT),-TRunoff%etout(iunit,nt) *0.3_r8)))
    ...
    subroutine mainchannelRoutingDOM(iunit,nt,ntdom,theDeltaT)
      ...
      Tdom%domRout(iunit,ntdom) = min((Tdom%domR(iunit,ntdom)+(Tdom%domRUp(iunit,ntdom) + Tdom%domToutLat(iunit,ntdom))* theDeltaT)/theDeltaT,max(0._r8,min(-TRunoff%erout(iunit,nt) * (Tdom%domR(iunit,ntdom) + (Tdom%domRUp(iunit,ntdom) + Tdom%domToutLat(iunit,ntdom)) * theDeltaT)/(max(0._r8,TRunoff%wr(iunit,nt)-TRunoff%dwr(iunit,nt)*theDeltaT)+(TRunoff%erlateral(iunit,nt)+TRunoff%erin(iunit,nt)+temp_gwl)*theDeltaT),-TRunoff%erout(iunit,nt)*0.3_r8)))
