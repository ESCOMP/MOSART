...
!========================================================================
   ...
   subroutine init_decomp(this, locfn, decomp_option, use_halo_option, &
      ...
      !-------------------------------------------------------
      ! Determine glo2gdc (global to local) and gdc2glo (local to global)
      !-------------------------------------------------------
      ...
      do nr = 1,nlon*nlat
         procid = pocn(nr)
         if (procid >= 0) then
            glo2gdc(nr) = nrs(procid) + nba(procid)
      ...
      ! Determine gdc2glo - local to global index space
      do j = 1,nlat
         do i = 1,nlon
            n = (j-1)*nlon + i
            nr = glo2gdc(n)
      ...
      do nr = begr,endr
         ...
         if (dnID_global(n) > 0) then
            if (glo2gdc(dnID_global(n)) == 0) then
               write(iulog,*) subname,' ERROR glo2gdc dnID_global ',&
                    nr,n,dnID_global(n),glo2gdc(dnID_global(n))
   ...
   subroutine set_halo_index(global_index, halo_index, glo2gdc, nr, begr, endr, pocn, store_halo_index, halo_arrayptr_index)

      ! Arguments
      integer, intent(in)    :: global_index
      integer, intent(in)    :: halo_index
      integer, intent(in)    :: glo2gdc(:)
      ...
      if (pocn(global_index) /= iam) then
         ...
         halo_arrayptr_index(nr-begr+1,halo_index) = glo2gdc(global_index) - begr + 1
      end if
